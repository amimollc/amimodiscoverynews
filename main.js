    // ===================== MAIN.JS (complete with bottom bar & saved tab) =====================
    (function() {
        // ---------- GLOBAL DATA ----------
        let allArticles = [];
        ...
        // RSS FEEDS & local map (keep original exactly)
        const WORLD_FEEDS = [
        ...
        async function fetchFeed(cfg) { try { let resp=await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(cfg.url)}`); let data=await resp.json(); if(data.status!=='ok') return []; return data.items.slice(0,12).map(item=>({ title:item.title, link:item.link, pubDate:item.pubDate, description:(item.description||"").replace(/<[^>]*>/g,'').substring(0,200), source:cfg.name, category:cfg.category, imageUrl:item.thumbnail||cfg.imgFallback })); } catch(e){ return []; } }
        ...
        function renderNewsFeed() {
            ...
            toRender.forEach((art,i)=>{
                ...
                if((i+1)%5===0) html+=`<div class="inline-ad"><i class="fas fa-ad"></i> Advertisement — Support Amimo</div>`;
            });
            feedDiv.innerHTML=html; attachSaveEvents();
        }
        function attachSaveEvents(){ document.querySelectorAll('.save-btn').forEach(btn=>{ btn.removeEventListener('click',saveHandler); btn.addEventListener('click',saveHandler); }); }
        function saveHandler(e){
            ...
            updateSavedCounter(); if(currentView==='saved') renderSavedArticles();
        }
        ...
        function renderSavedArticles(){
            ...
            document.querySelectorAll('.unsave-btn').forEach(btn=>{
                btn.addEventListener('click',(e)=>{
                    ...
                    updateSavedCounter();
                    renderSavedArticles();
                    if(currentView==='home') attachSaveEvents(); // refresh home button states
                    else attachSaveEvents();
                    showToast('Article removed from saved');
                });
            });
        }
        
        function renderTrendingCarousel(){ /* identical to original shortened */
            let cats=['Politics','Technology','Sports','Entertainment','Business','Health','World','Local'];
            let selected=[]; for(let cat of cats){ let art=allArticles.filter(a=>a.category===cat).slice(0,2); selected.push(...art); }
            ...
            document.querySelectorAll('.save-trend').forEach(btn=>{ btn.addEventListener('click',()=>{ if(!savedArticles.some(s=>s.link===btn.dataset.link)){ savedArticles.push({ title:btn.dataset.title,link:btn.dataset.link,imageUrl:btn.dataset.img,source:btn.dataset.source,description:btn.dataset.desc }); localStorage.setItem("amimo_saved",JSON.stringify(savedArticles)); btn.innerHTML='✅ Saved'; updateSavedCounter(); if(currentView==='saved') renderSavedArticles(); } }); });
        }
        
        async function fetchMoreForCategory(cat){ /*background fetch minimal*/ return 0; }
        function switchCategory(cat){ if(currentCategory===cat)return; currentCategory=cat; document.querySelectorAll('.cat-pill').forEach(p=>p.classList.remove('active')); let activePill=Array.from(document.querySelectorAll('.cat-pill')).find(p=>p.dataset.cat===cat); if(activePill)activePill.classList.add('active'); applyCategoryFilter(); window.scrollTo({top:0}); }
        async function detectLocation(){ try{ let res=await fetch('https://ipapi.co/json/'); let d=await res.json(); if(d.country_code){ userCountry=d.country_code; userCountryName=d.country_name||userCountry; } }catch(e){ userCountry="ZM"; userCountryName="Zambia"; } document.getElementById('countryBadge').innerHTML=`<i class="fas fa-map-marker-alt"></i> ${userCountryName}`; }
        ...
        function initScrollObserver(){
            if(scrollObserver) scrollObserver.disconnect();
            scrollObserver=new IntersectionObserver(async(entries)=>{ if(entries[0].isIntersecting && !isLoadingMore && currentView==='home') { if(displayLimit<currentFiltered.length){ displayLimit=Math.min(displayLimit+25,currentFiltered.length); renderNewsFeed(); } } },{threshold:0.1});
    ...
    // Service worker (original kept)
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', async () => {
            try { const registration = await navigator.serviceWorker.register('sw.js'); console.log('SW registered'); if ('Notification' in window) await Notification.requestPermission(); } catch(err) { console.log('SW failed'); }
        });
    }
